...
ALMA is the application's visible face, not a separate process, model, evidence
source, or authority. Her animation and accessible status text must reflect
real task state. She must never imply that an operation succeeded, a claim is
supported, or a task was cancelled unless the validated runtime state says so.
